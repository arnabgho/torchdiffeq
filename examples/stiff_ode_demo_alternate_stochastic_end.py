...
parser.add_argument('--skip_proportion', type=float, default=0)
...
if args.adjoint:
    ...
    from torchdiffeq import odeint_adjoint_stochastic_end as odeint_stochastic_end
else:
    from torchdiffeq import odeint_stochastic_end
...
if args.viz:
    makedirs('png_alternate_stochastic_end')
...
def visualize(true_y, pred_y, pred_ys, odefunc, itr):
    ...
    if args.viz:
        ...
        fig.tight_layout()
        plt.savefig('png_alternate_stochastic_end/{:04d}'.format(itr))
        plt.draw()
        plt.pause(0.001)
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.niters + 1):
        ...
        pred_y = odeint_stochastic_end(func, batch_y0, batch_t,num_skips=args.num_skips,skip_proportion=args.skip_proportion)
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                ...
                for i in range(args.ntest):
                    pred_ys.append(odeint_stochastic_end(func,true_y0,t,num_skips=args.num_skips,skip_proportion=args.skip_proportion))
